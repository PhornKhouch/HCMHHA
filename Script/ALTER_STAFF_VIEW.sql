...
SP.EmpType,SP.Division,SP.JobCode,SP.LevelCode,SP.Office,SP.Groups,CardNo,SP.Title,SP.Manager,
...
HRBranch.Description Branch,PRParameter.Description AS PayParameter,PostFamily,SP.GroupDept,SECT,DEPT,SP.CATE,HRDivision.Description DivisionDesc,
LO.Description as Location,SP.LOCT,HRNation.Description as Nationality,SP.TeleGroup,
...
(CASE WHEN SP.Sex='M' then N'?????' else N'????' end) SexKH,DATEDIFF(year,DOB,GETDATE()) Age,
...
(CASE WHEN SP.Title='Mr' then N'???' when sp.Title='Madam' then N'???????'
When sp.Title='Miss' then N'?????' When sp.Title='Ms' then N'????????' else N'????????' end) TitleKH,IsPayPartial,
