...
namespace HR.Controllers.PR.PRM
{
    ...
    public class PRApproveSalaryFPController : Humica.EF.Controllers.MasterSaleController
    {
        ...
        public ActionResult Index()
        {
            ...
            BSM.ListAppSalaryPending = DB.HisPendingAppSalaryFPs.Where(w => w.IsLock == false).ToList();
        ...
        public ActionResult Create(int InYear, int InMonth)
        {
            ...
            UserConfListAndForm();
            PRFirstPaySalaryGeneration BSM = new PRFirstPaySalaryGeneration();
            var SalaryPending = DB.HisPendingAppSalaryFPs.Where(w => w.InYear == InYear && w.InMonth == InMonth && w.IsLock == false).ToList();
        ...
        [HttpPost]
        public ActionResult Create(PRFirstPaySalaryGeneration BSM, string id)
        {
            UserSession();
            //DataSelector();
        ...
        #endregion
        public ActionResult Download(int InYear, int InMonth)
        {
            ActionName = "Index";
            UserSession();
            UserConfForm(SYActionBehavior.EDIT);
            var report = DB.HisPendingAppSalaryFPs.Where(w => w.InYear == InYear && w.InMonth == InMonth).ToList();
            if (report.Count > 0)
            {
                RptMonthlyPay sa = new RptMonthlyPay();
                DateTime ToDate = new DateTime(InYear, InMonth, 1);
                var objRpt = DB.CFReportObjects.Where(w => w.ScreenID == "RPTPR00002"
                    && w.DocType == "Details" && w.IsActive == true).ToList();
                if (objRpt.Count > 0)
                {
        ...
        public ActionResult SelectParam(string docType)
        {
            ...
            {
                BSM = (PRFirstPaySalaryGeneration)Session[Index_Sess_Obj + ActionName];
                BSM.SetAutoApproval("PRM0000022", docType);
                Session[Index_Sess_Obj + ActionName] = BSM;
            }
        ...
        {
            ViewData["DOCUMENT_TYPE"] = DB.ExCfWorkFlowItems.Where(w => w.ScreenID == SCREEN_ID || w.ScreenID == "PRM0000022").ToList();
