...
{
    public class RPTNSSFReportController : Humica.EF.Controllers.MasterSaleController
    {

        private const string SCREEN_ID = "RPTPR00006";
        private const string URL_SCREEN = "/Reporting/RPTNSSFReport/";
        private string Index_Sess_Obj = SYSConstant.BUSINESS_OBJECT_MODEL + SCREEN_ID;
        private string ActionName = "";
        HumicaDBContext DB = new HumicaDBContext();
        SMSystemEntity DBA = new SMSystemEntity();
        public RPTNSSFReportController()
            : base()
        {
        ...
        public ActionResult DocumentViewerPartial()
        {
            ...
            try
            {
                if (Session[Index_Sess_Obj + ActionName] != null)
                {
                    ...
                    if (RPT.ReportTypes == "Latter")
                    {
                        sa = new RPTNSSFLatter();
                    }
                    else if (RPT.ReportTypes == "EFrom")
                    {
                        sa = new RptEForm();
                    }
                    else if (RPT.ReportTypes == "Monthly")
                    {
                        sa = new RptNSSFList();
                    }
                    else if (RPT.ReportTypes == "Summary")
                    {
                        sa = new RptNSSFSummary();
                    }
        ...
        public ActionResult DocumentViewerExportTo()
        {
            ...
            {
                var Filter = (FTFilterEmployee)Session[Index_Sess_Obj + ActionName];
                var reportModel = (XtraReport)Session[Index_Sess_Obj];
