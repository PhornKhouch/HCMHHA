...
//-------------------Commpany Code------------------------------------
function OnSelectionChangedCompany(s, e) {
    s.GetSelectedFieldValues("Company", GetSelectedFieldValuesCallbackCompany);
}
function GetSelectedFieldValuesCallbackCompany(values) {
    _selectedIDsCompany = "";

    for (var index = 0; index < values.length; index++) {
        _selectedIDsCompany += values[index] + textSeparator;

    }
    if (_selectedIDsCompany.length > 0) {
        _selectedIDsCompany = _selectedIDsCompany.substring(0, _selectedIDsCompany.length - 1);
    }

    checkComboBoxCompany.SetText(_selectedIDsCompany);
}
function clearSelectedCompany(s, e) {
    checkComboBoxCompany.SetText("");
    CompanyGridListSelector.UnselectAllRowsOnPage();
    _selectedIDsCompany = "";
